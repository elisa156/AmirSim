...
function parseAnswerKeySections(pagesText) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    /* MIRRORED inline key (pdf.js RTL): ') 2 (  . 1' = question 1, answer 2. */
    /* CURRENT NULL GUARD: a key row can arrive before any header — don't crash */
    if (inEnglish && !current) current = {};
    if (!current) current = {};   /* mirrored rows outside English sections too */
...
async function handleFiles(fileList) {
  ...
  /* PHONE FIX: Android/Drive files often lack the .pdf extension — classify by
     MIME type first, extension as fallback. Unknown-but-PDF-typed files pass. */
  const isPdf = f => (f.type === 'application/pdf' || /\.pdf$/i.test(f.name || '') ||
                      (f.name || '').toLowerCase().includes('pdf') || (!f.type && !/\./.test(f.name || '')));
  const isVocab = f => /\.(csv|tsv|txt)$/i.test(f.name || '') || /^(text\/csv|text\/plain)$/.test(f.type || '');
  const vocabFiles = all.filter(isVocab);
